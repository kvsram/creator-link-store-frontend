...
function Home({data}){const m=data.metrics;return <>
  ...
  {!data.store.payouts_enabled&&<div className="notice"><b>External real-money setup required.</b> Connect a test Razorpay account and verify webhooks before accepting INR payments.<button>Open settings</button></div>}
...
function PublicStore(){const handle=location.pathname.replace(/^\//,'')||'alex';const [page,setPage]=useState(null),[missing,setMissing]=useState(false);useEffect(()=>{request(`/api/public/${handle}`).then(setPage).catch(()=>setMissing(true))},[handle]);if(missing)return <Signup/>;if(!page)return <div className="public-loading">Loading store…</div>;return <main className="public-store"><div className="public-profile"><div className="avatar">{page.creator.display_name[0]}</div><h1>{page.creator.display_name}</h1><p>@{page.creator.handle}</p><p>{page.creator.bio}</p></div><div className="public-products">{page.products.map(p=><article key={p.id}><div className="public-art">{p.type==='meeting'?'◷':'▤'}</div><div><h2>{p.title}</h2><p>{p.description}</p><b>{money(p.price_subunits,page.store.currency)}</b></div><CheckoutAction product={p}/></article>)}</div>{page.links.map(l=><a className="public-link" key={l.id} href={l.url}>{l.title}<span>↗</span></a>)}<small>Powered by Creator Store · External payments clearly confirmed before charge</small></main>}

function CheckoutAction({product}){const [message,setMessage]=useState('');const start=async()=>{setMessage('Creating secure checkout…');try{const session=await request('/api/v1/checkout/sessions',{method:'POST',headers:{'Idempotency-Key':crypto.randomUUID()},body:JSON.stringify({creatorId:1,productId:product.id})});if(session.provider==='stripe'){location.assign(session.redirect_url);return}await loadScript(session.checkout_script);const checkout=new window.Razorpay({key:session.public_key_id,amount:session.amount_subunits,currency:session.currency,order_id:session.provider_session_id,name:'Creator Store',description:product.title,handler:async result=>{await request('/api/v1/payments/razorpay/verify',{method:'POST',body:JSON.stringify({orderId:result.razorpay_order_id,paymentId:result.razorpay_payment_id,signature:result.razorpay_signature})});setMessage('Payment response verified. Final confirmation arrives by webhook.')}});checkout.open();setMessage('Razorpay checkout opened.') }catch(e){setMessage(e.message)}};return <div className="checkout-action"><button onClick={start}>Buy securely</button>{message&&<small>{message}</small>}</div>}
